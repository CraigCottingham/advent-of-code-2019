defmodule AoC.Day10.Spec do
  ...
  example_group "angle/2" do
    it("positive Y axis",
      do: expect(AoC.Day10.angle({0, 0}, {-1, 0})) |> to(eq(Math.pi() * 0 / 4))
    )

    it("quadrant I", do: expect(AoC.Day10.angle({0, 0}, {-1, 1})) |> to(eq(Math.pi() * 1 / 4)))

    it("positive X axis", do: expect(AoC.Day10.angle({0, 0}, {0, 1})) |> to(eq(Math.pi() * 2 / 4)))

    it("quadrant IV", do: expect(AoC.Day10.angle({0, 0}, {1, 1})) |> to(eq(Math.pi() * 3 / 4)))

    it("negative Y axis", do: expect(AoC.Day10.angle({0, 0}, {1, 0})) |> to(eq(Math.pi() * 4 / 4)))

    it("quadrant III", do: expect(AoC.Day10.angle({0, 0}, {1, -1})) |> to(eq(Math.pi() * 5 / 4)))

    it("negative X axis",
      do: expect(AoC.Day10.angle({0, 0}, {0, -1})) |> to(eq(Math.pi() * 6 / 4))
    )
